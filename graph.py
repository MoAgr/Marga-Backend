...
class Graph:
    ...
    def add_route(self,db:Session,name,yatayat,vehicle_types,route,username,geojson,approved=False): 
        ...
        for curr_node in route:
            ...
            elif crud.get_node_in_range(db,(curr_node["lat"]-0.0002,curr_node["lat"]+0.0002),
                                        (curr_node["lng"]-0.0002,curr_node["lng"]+0.0002)):
                route_nodes.append(crud.get_node_in_range(db,(curr_node["lat"]-0.0002,curr_node["lat"]+0.0002),
                                        (curr_node["lng"]-0.0002,curr_node["lng"]+0.0002)))
            else:
